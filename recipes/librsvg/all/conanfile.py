...
class LibrsvgConan(ConanFile):
    ...
    default_options = {
        ...
        "with_vapigen": False,  # currently not on conancenter
    }
    ...
    def requirements(self):
        ...
        if self.options.with_gdk_pixbuf or self.options["pixbuf-loader"]:
    ...
    def generate(self):
        ...
        tc.project_options["docs"] = feature(self.options.with_gidocgen)
        tc.project_options["vala"] = feature(self.options.with_vapigen)
        tc.project_options["tests"] = true_false(self.options.tests)
        tc.project_options["avif"] = feature(self.options.avif)
        tc.project_options["pixbuf"] = feature(self.options.with_gdk_pixbuf or self.options["pixbuf-loader"])
        tc.project_options["pixbuf-loader"] = feature(self.options["pixbuf-loader"])
        if self.options.triplet:
            tc.project_options["triplet"] = str(self.options.triplet)
        if self.options.rustc_version:
            tc.project_options["rustc-version"] = str(self.options.rustc_version)
        tc.generate()
    ...
    def package_info(self):
        ...
        if is_msvc(self):
            self.cpp_info.system_libs += ["ntdll", "userenv"]

        if self.options.avif:
            self.cpp_info.requires += ["dav1d::dav1d"]
